...
public class SyncPropertiesTest {
  ...
  public void testPersist() throws Exception {
    ...
    sp1.setDateTime(SyncWorker.PROP_SYNC_START, start);

    ZonedDateTime end = ZonedDateTime.now();
    sp1.setDateTime(SyncWorker.PROP_SYNC_END, end);

    ZonedDateTime asOfDateTime = null;
    sp1.setDateTime(SitemapCollector.PROP_AS_OF_DATE_TIME, asOfDateTime);
    ...
    assertThat(sp2.getDateTime(SyncWorker.PROP_SYNC_START), equalTo(start));
    assertThat(sp2.getDateTime(SyncWorker.PROP_SYNC_END), equalTo(end.withZoneSameInstant(ZoneOffset.UTC)));
    assertThat(sp2.getDateTime(SitemapCollector.PROP_AS_OF_DATE_TIME), is(asOfDateTime));
  }
}
